...
from app.abs.llm_direct.prompts import ROUGH_SELECT_LINES_PROMPT, TASK_DESCRIPTION_PROMPT, IDENTIFY_ELEMENTS_PROMPT
...
class IdentifyState(PromptState):
    ...
    def accept(self):
        ...
        _identify_elements_prompt = [{"role": "user", "content": IDENTIFY_ELEMENTS_PROMPT}]
        _messages_copy.extend(_identify_elements_prompt)
        valid, response = self.invoke_validate_retry(_messages_copy)
        if valid:
            _identify_elements_prompt.append({"role": "assistant", "content": response})
            self.analyzer.global_history.identify_elements_history = _identify_elements_prompt
            key_elements = IdentifyState.extract_critical_elements(response)
            self.analyzer.key_elements = key_elements

        self.exit()
        # 转到 MatchState 进行精确匹配
        self.analyzer.prompt_state = MatchState(self.analyzer)
        return
...
class MatchState(PromptState):
    """
    将 LLM 识别的元素精确匹配到 pattern_input.tree 上的节点。

    匹配规则：
    - value 完全匹配（忽略大小写）
    - 忽略 LLM 的 type（因为 LLM 可能识别错误）
    - 行号在前后 1 行范围内
    """

    def accept(self):
        for element in self.get_all_elements(self.analyzer.pattern_input.tree):
            if self.match_element(element, self.analyzer.key_elements):
    ...
    def match_element(self, tree_element: Dict, llm_elements: List[Dict]) -> bool:
        """
        检查 tree_element 是否与任一 llm_element 精确匹配

        匹配条件：
        1. value 完全相同（忽略大小写）
        2. 行号在前后 1 行范围内
        """
        tree_value = tree_element.get("value", "").lower()
        tree_start = tree_element.get("start_line")
        tree_end = tree_element.get("end_line")

        for llm_elem in llm_elements:
            llm_value = llm_elem.get("value", "").lower()
            llm_start = llm_elem.get("start_line")
            llm_end = llm_elem.get("end_line")

            # value 完全匹配
            if tree_value != llm_value:
                continue

            # 行号在前后 1 行范围内
            if (tree_start - 1 <= llm_start <= tree_end + 1 or
                tree_start - 1 <= llm_end <= tree_end + 1 or
                llm_start - 1 <= tree_start <= llm_end + 1):
                return True
