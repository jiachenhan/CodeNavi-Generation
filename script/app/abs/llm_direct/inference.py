...
import utils
from app.abs.llm_direct.history import GlobalHistories, MessageList
from app.abs.llm_direct.prompt_state import InitialState, ExitState
...
class Analyzer:
    """
    LLM 直接分析器：使用精确匹配代替相似度匹配。

    与 llm_4_round 的区别：
    - 使用 MatchState 进行精确匹配（value 完全相同 + 行号前后1行）
    - 忽略 LLM 识别的 type（因为 LLM 可能识别错误）
    """
    def __init__(self,
                 llm: LLMAPI,
        ...
        self.key_elements = []  # LLM 直接识别的关键元素
        self.considered_elements = set()  # 匹配到的 AST 节点 ID
...
if __name__ == "__main__":
    ...
    for case in cases:
        ...
        _logger.info(f"finished case: {case}\n")
        exit(0)