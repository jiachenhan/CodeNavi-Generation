...
def java_llm_abstract(timeout_sec: float,
                      # LLM的抽象结果抽象pattern
                      origin_pattern_path: Path,
                      abstract_info_path: Path,
                      abstract_pattern_path: Path,
                      java_program: str):
    _logger.info(f"Start abstract pattern leveraging llm for {origin_pattern_path.stem}")
    work_dir = utils.config.get_root_project_path()
...
def java_genpat_abstract(timeout_sec: float,
                         # LLM的抽象结果抽象pattern
                         origin_pattern_path: Path,
                         abstract_pattern_path: Path,
                         java_program: str):
